...
import amber.lp.Main as Main
import amber.lp.Preprocess as Preprocess
import amber.lp.PossibleChar as PossibleChar
...
def loadKNNDataAndTrainKNN():
    ...
    try:
        npaClassifications = np.loadtxt("amber/lp/classifications.txt", np.float32)                  # read in training classifications
    ...
    try:
        npaFlattenedImages = np.loadtxt("amber/lp/flattened_images.txt", np.float32)                 # read in training images
    except:                                                                                 # if file could not be opened
